...
def test_daily_report_request_accepts_wrapper_fields(tmp_path: Path) -> None:
    request = DailyReportRequest(
        ...
        product_models=["M678"],
        source_files={"daily_yield": tmp_path / "source.xlsx"},
    ...
    assert request.source_files["daily_yield"] == tmp_path / "source.xlsx"
    assert request.generator_root == tmp_path / "generator"
