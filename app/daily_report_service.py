...
def default_report_output_dir() -> Path:
    """Return the Agent delivery directory from validated configuration."""
    configured = ConfigLoader().get().agent.daily_report.output_dir.strip()
    if not configured:
...
def generate_daily_report(
    ...
    *,
    ...
    del preflight  # Compatibility argument; public CLI owns its own preflight checks.
    ...
    request = DailyReportRequest(
        ...
        generator_root=form.generator_root,
        generator_workspace=form.generator_workspace,
