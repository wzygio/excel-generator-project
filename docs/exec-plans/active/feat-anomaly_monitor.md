...
# Task
我需要使用V-Agent（阿里开发的一个智能体平台）来接受异常HL的内容。即：
1. 当点击“异常HL”按钮时，除了在本地生成一份文件外。API应该发送一个post给Agent，将格式化之后的“HL异常”推送出去。
2. Agent在接收后，将“HL异常”推送到群里。

# Workflow
1. 请开发相应API
2. 请查看图中的Agent设置，告诉我基于你第一步开发的API，我应该如何如何设置？
3. 因为你无法控制V-Agent平台，所以请告诉我一套测试流程，让我可以检测是否成功